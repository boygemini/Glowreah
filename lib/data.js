...
const getCollectionNamesForHumans = () => {
  const rawCollections = JSON.parse(sessionStorage.getItem("db_collections"));
  ...
  function switchToHumanReadable(key) {
    switch (key) {
      case "skirts":
        return { name: capitalizeFirstLetter(key), image: "" };
      case "tops":
        return { name: capitalizeFirstLetter(key), image: "" };
      case "pants":
        return { name: capitalizeFirstLetter(key), image: "" };
      ...
      case "blousesandshirts":
        return { name: "Blouses and Shirts", image: "" };
      case "jeans":
        return { name: capitalizeFirstLetter(key), image: "" };
      case "sweatersandcardigands":
        return { name: "Sweaters and Cardigans", image: "" };
      case "jackets":
  ...
  return rawCollections.map((name) => switchToHumanReadable(name));
};
